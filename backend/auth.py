...
AUTH0_DOMAIN = 'dev-by-youssef.eu.auth0.com'
ALGORITHMS = ['RS256']
...
def check_permissions(permission, payload):
    if payload is None:
        raise AuthError("no payload provided", 401)

    if "permissions" not in payload:
        raise AuthError("permissions missing in payload", 401)

    if permission not in payload['permissions']:
        raise AuthError("you have no access", 401)
    return True
